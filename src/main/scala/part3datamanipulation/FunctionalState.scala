...
object FunctionalState {
  ...
  def addToCart2(item: String, price: Double): State[ShoppingCart, Double] = State { cart =>
    (ShoppingCart(item :: cart.items, cart.total + price), price + cart.total)
  }

  val danielsCart: State[ShoppingCart, Double] = for {
    _ <- addToCart2("Fender guitar", 500)
    _ <- addToCart2("Elixir strings", 19)
    total <- addToCart2("Electric cable", 8)
  } yield total
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(danielsCart.run(ShoppingCart(List(), 0.0)).value)
  }
